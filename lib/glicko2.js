(function (exports) {
  ...
  Player.prototype.getOutcomes = function (t) {
    return this.outcomes[t];
  };
  ...
  Player.prototype._makef = function (delta, v, a) {
    return function (x) {
      return (
        (Math.exp(x) *
          (Math.pow(delta, 2) - Math.pow(this.__rd, 2) - v - Math.exp(x))) /
          (2 * Math.pow(Math.pow(this.__rd, 2) + v + Math.exp(x), 2)) -
  ...
  function Glicko2(settings) {
    ...
    this._volatility_algorithm =
      volatility_algorithms[settings.volatility_algorithm || "newprocedure"];
  ...
  Glicko2.prototype.addMatch = function (player1, player2, outcome) {
    var pl1 = this._createInternalPlayer(
      player1.rating,
      player1.rd,
      player1.vol,
      player1.id
    );
    var pl2 = this._createInternalPlayer(
      player2.rating,
      player2.rd,
      player2.vol,
      player2.id
