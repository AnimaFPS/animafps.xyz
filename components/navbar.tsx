...
export class Navbar extends React.Component<IProps, IState> {
  ...
  checkPageLink = (title:string, path:string) => {
    if (path === this.props.path) {
      ...
      return (<NextLink href={path} key={title}>
        <Link component="button" variant="button" style={{marginRight: "10px"}} color="inherit" className="link">
          {title}
        </Link>
      </NextLink>)
    }
            ...
            <AppBar position="static" color="primary" elevation={0}>
                <Toolbar style={{flexWrap: 'wrap'}}>
                    ...
                    <Hidden mdUp>
                      ...
                      <Drawer anchor="top" open={this.state.top} onClose={this.toggleDrawer}>
